...
#define WHEELS_BIND_TYPE_WITH_MX_CLASSID(T, id) \
  ...
  /*  template <class ShapeT, class DPT>
    ...
    public:
        ...
        void set_field(const std::string & name, const matlab_mxarray & a, int i = 0) const;
        ...
        void set_at_subs(const T & v, Ints ... subs) {
            assert(is<T>());
    ...
    template <class T>
    class matlab_mxarray_of_type : public matlab_mxarray {
    ...
    public:
        ...
        bool set_var(const std::string & name, const matlab_mxarray & mxa, bool as_global = false);
    ...
    class matlab_engine {
    public:
        matlab_engine(const std::string & defaultDir = std::string(), bool singleUse = false);
        ~matlab_engine();
